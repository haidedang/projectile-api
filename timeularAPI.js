...
exports.initializeToken = async tokenApi => {
    try {
        ...
        winston.error(
            'timeularAPI No token file seems to be available. Please run "hostname:port/start" to create ' +
                'a token file.'
        );
        // process.exit();
    }
...
exports.getActivities = async () => {
    const result = await rp({
        ...
        .catch(function(err) {
            winston.warn(
                "getActivities -> Something went wrong. Here's the body: ",
                res.body
            );
...
async function compareActivities() {
    ...
    activityList.activities.forEach(item => {
        ...
        projectileJobList.forEach(item2 => {
            const timeularActivityID = item.name.substring(
    ...
    projectileJobList.forEach(item => {
        ...
        activityList.activities.forEach(item2 => {
            const timeularActivityID = item2.name.substring(
                item2.name.lastIndexOf('[') + 1,
                item2.name.lastIndexOf(']')
            );
...
exports.packageActivityList = async id => {
    ...
    timeularActivities.activities.forEach(item => {
        ...
        obj['Package'] = item.name.substring(
            item.name.lastIndexOf('[') + 1,
            item.name.lastIndexOf(']')
        );
    ...
    let packageId = '';
    if (id.includes('-')) {
        // projectile package format --> translate to activity
        packageId = id;
        ...
        // timeular package format --> translate to package
        activityId = id;
        packageActivity.forEach(item => {
...
exports.updateActivities = async (create, archive) => {
    ...
    if (create) {
        result.projectileHasTimeularNot.forEach(item => {
            winston.debug(
                'Starting creating activities...',
                item.name,
                item.no
            );
            if (!createActivity(item.name, item.no)) {
                winston.error(
                    'Error: Creating activity for ' + item.name,
                    item.no + ' failed.'
                );
                resultState = false;
            }
    ...
    if (archive) {
        result.timeularHasProjectileNot.forEach(item => {
            winston.debug(
                'Starting archiving activities...',
                item.name,
                item.id
            );
            if (!archiveActivity(item.id)) {
                winston.error(
                    'Error: Archiving of activity ' + item.name,
                    item.id + ' failed.'
                );
                resultState = false;
            }
...
async function getSumMinutes(duration) {
    ...
    return sumMinutes;
} // WIP!!! Alternative with time like 9:00 as minimum start time?! // WIP - successor to bookActivity
 ...
 */ exports.bookActivityNG = async ({ date, duration, activityId, note }) => {
    ...
    if (timeEntries && timeEntries.length > 0) {
        timeEntries.forEach(item => {
            if (new Date(item.duration.stoppedAt + 'Z') > latestStoppedAt) {
                winston.debug(
                    'Debug bookActivity -> lastestStoppedAt -> neuer gefunden!'
                );
    ...
    newStoppedAt = new Date(
        newStoppedAt.setMinutes(newStoppedAt.getMinutes() + sumMinutes)
    );
    winston.debug(
        latestStoppedAt.toISOString().substr(0, 23),
        newStoppedAt.toISOString().substr(0, 23)
    );
...
exports.merge = async (startDate, endDate) => {
    ...
    await normalizeUP(startDate, endDate, monthCleaned).then(async result => {
        winston.debug(
            'Merge (Range) -> monthCleaned size in normalizeUP: ' +
                monthCleaned.length
        );
        winston.debug('Merge (Range) -> normalizeUP: ');
        winston.debug('Merge (Range) -> Input: ' + util.inspect(result));
        if (result && result.length <= 0) {
            // nothing to do, no need to call saveToProjectile
            winston.debug(
                'Merge (Range) -> normalized list empty - nothing to do.'
            );
...
async function markEntriesWithLimit(monthArray, limitPackageArrayFromServer) {
    ...
    for (let i = 0; i < limitPackageArrayFromServer.length; i++) {
        for (let j = 0; j < monthArray.length; j++) {
            ...
            winston.debug(
                'CHECK: ' + monthArray[j]['Activity'],
                limitPackageArrayFromServer[i]['no'],
                monthArray[j]['Activity'] ===
                    limitPackageArrayFromServer[i]['no']
            );
            if (
                monthArray[j]['Activity'] ===
...
async function saveToProjectile(monthArray) {
    ...
    const limitPackageArrayFromServer = await projectile.joblistLimited(
        data,
        'limitTime',
        item => {
            return item > 0;
    ...
    winston.debug(
        'saveToProjectile -> after projectile.joblistLimited -> limitPackageArrayFromServer: ',
        JSON.stringify(limitPackageArrayFromServer, null, 2)
    );
    const packageReply = await markEntriesWithLimit(
        monthArray,
        limitPackageArrayFromServer
    );
    winston.debug(
        'saveToProjectile -> after markEntriesWithLimit (improved) -> packageReply: ',
    ...
    async function saving(packageReply) {
        ...
        for (let i = 0; i < packageReply.length; i++) {
            ...
            if (packageReply[i]['Limit']) {
                ...
                const data = await projectile.fetchNewJobList();
                const projectileObject = await projectile.joblistLimited(
                    data,
                    'no',
                    item => {
                ...
                winston.debug(
                    'duration of new entry: ' +
                        packageReply[i].Duration +
                        ' remainingTime in package before add: ' +
                ...
                if (
                    parseFloat(packageReply[i].Duration.toFixed(5)) <=
                    parseFloat(
                        Number(projectileObject[0].remainingTime.toFixed(5))
                    )
                ) {
                    const response = await projectile.save(
                        packageReply[i]['StartDate'],
                        parseFloat(packageReply[i]['Duration'].toFixed(5)),
                        packageReply[i]['Activity'],
                        packageReply[i]['Note']
                    );
                    winston.debug(
                        'saving w/ limit: ' + packageReply[i]['StartDate'],
                        packageReply[i]['Duration'],
                        packageReply[i]['Activity'],
                        packageReply[i]['Note']
                    );
                    ...
                    obj['Result'] = 'negative';
                    winston.warn(
                        'Saving package with limit failed - Remaining Time exceeded. ' +
                            packageReply[i]['StartDate'],
                        packageReply[i]['Duration'],
                        packageReply[i]['Activity'],
                        packageReply[i]['Note'] +
                            ' with remaining time of: ' +
                            Number(projectileObject[0].remainingTime)
                    );
                }
            } else {
                // false --> without limit! empty notes can occur
                if (packageReply[i]['Activity'] !== '') {
                    ...
                    winston.debug(
                        'saving w/o limit: ' + packageReply[i]['StartDate'],
                        packageReply[i]['Duration'],
                        packageReply[i]['Activity'],
                        packageReply[i]['Note']
                    );
    ...
    winston.debug(
        'saveToProjectile -> after everything -> gesResult: ',
        JSON.stringify(gesResult, null, 2)
    );
    return {
        gesResult
    };
...
async function getDistinctProjectileRange(startDate, endDate) {
    try {
        ...
        const List = await projectile.getallEntriesInTimeFrame(
            startDate,
            endDate
        );
        /* TODO bugfix!
    ...
    */
        ...
        TimeRangeArray.forEach(item => {
            const entryObj = {};
            entryObj['StartDate'] =
                item[item.map(item => item.n).indexOf('Day')]['v'];
            entryObj['Duration'] =
                item[item.map(item => item.n).indexOf('Time')]['v'];
            entryObj['Activity'] =
                item[item.map(item => item.n).indexOf('What')]['v'];
            entryObj['Note'] =
                item[item.map(item => item.n).indexOf('Note')]['v'];
            temp.push(entryObj);
        });
...
async function normalizeUP(startDate, endDate, MonthCleaned) {
    ...
    winston.debug(
        'normalizeUP -> after splitintoSeperateDays: ',
        JSON.stringify(monthDay, null, 2)
    );
    // get DateRange of Projectile  [ [Day1],[Day2] ]
    const serverDays = await getDistinctProjectileRange(startDate, endDate);
    winston.debug(
        'normalizeUP -> after getDistinctProjectileRange : ',
        JSON.stringify(serverDays, null, 2)
    );
    ...
    monthDay.forEach(item => {
        clientDaysInProjectile.push(
            serverDays.filterAdvanced(
                obj => obj[0].StartDate === item[0].StartDate,
                temp => serverDays.indexOf(temp)
            )
        );
    });
    winston.debug(
        'normalizeUP -> monthDay.forEach(item): ',
        JSON.stringify(clientDaysInProjectile, null, 2)
    );
    // split serverArray in 2 Arrays - one containing the client days, one containing everything else
    const obj = await prepareForSaveAndDeleting(
        serverDays,
        clientDaysInProjectile
    );
    winston.debug(
        'normalizeUP -> prepareForSaveAndDeleting: ',
  ...
  */
    ...
    obj.dayList.forEach(async day => {
        day.forEach(async item => {
            winston.debug(
                'normalizeUP -> obj.dayList day item: ',
                item,
                item['Note'] ? item['Note'].includes(' #[') : ''
            );
            if (item['Note'] && !item['Note'].includes(' #[')) {
                winston.debug(
                    'normalizeUP -> Item not matching " #[" found! - booking in projectile only: ,  ',
                    item['Note']
                );
    ...
    winston.debug(
        'normalizeUP -> list w/ preserved projectile only entries: ',
        JSON.stringify(obj.dayList, null, 2)
    );
        ...
        for (let i = 0; i < serverArray.length; i++) {
           ...
           break;
            ...
            for (let j = 0; j < clientArray.length; j++) {
                ...
                winston.silly(
                    'compareV2 -> Strings getting compared: ',
                    JSON.stringify(clientArray[j]),
                ...
                if (
                    JSON.stringify(tempClientArrayEntry) ===
                    JSON.stringify(serverArray[i])
                ) {
          ...
          cleanProjectileList[i][j].Duration = null;
            ...
            if (cleanProjectileList[i][j].Duration !== null) {
                if (cleanProjectileList[i][j]['Note'].includes('#[')) {
                    ...
                    await projectile.delete(
                        cleanProjectileList[i][j].StartDate,
                        j
                    );
...
function prepareForSaveAndDeleting(serverDays, serverDaysInProjectile) {
    ...
    serverDaysInProjectile.forEach(item => {
        Overalllist[item] = true;
    }); // in brackets, doesn't return anything
