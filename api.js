...
app.post(basePath + '/start', async(req, res) => {
  ...
  if ((!json.projectileOnly && json.projectileUser && json.projectilePassword) || (json.projectileOnly &&
    json.projectileUser && json.projectilePassword && json.timeularApiKey && json.timeularApiSecret)){
    winston.debug('Credentials json from frontend not empty - trying to test and store those.');
    // set projectile creds
    const user = {login: json.projectileUser,
    ...
    async function testCredentials(user) {
      ...
      let result = rp(options, function(error, response, body) {
        ...
        winston.debug('testCredentials -> Login keyword existence after login attempt: ',
          response.body.substr(1, 2000).includes('Login</title>'));
        return result;
      }).then((result) => {
    ...
    async function retrieveToken(json) {
      ...
      const timeularApi = await rp.post('https://api.timeular.com/api/v2/developer/sign-in', {
        ...
        if (err) {
          winston.warn('retrieveToken -> An error occured while retrieving the token, check the error message: ');
          throw err;
        }
      }).catch((err) => {
      ...
      await retrieveToken(json).then((timeularApi) => {
        ...
        if (timeularApi) {
          ...
          fs.writeFile('timeularToken.txt', JSON.stringify(timeularApi2), (err) => {
            ...
            res.status(200).send(JSON.stringify({requestReceived: true, credsPresent, projectileOnly}));
          });
        } else {
