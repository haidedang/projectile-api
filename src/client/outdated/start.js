...
$(document).ready(function() {
  ...
  $("#buttonSetCred").click(function() {
    ...
    let $currentJson = {
      ...
      projectileOnly:
        $("#projectileOnly").attr("aria-pressed") === "true" ? true : false
    };
    ...
    $.post("//localhost:{port}/start/", $currentJson, function(data) {
      ...
      if (
        dataJSON.requestReceived &&
        dataJSON.credsPresent &&
        !dataJSON.projectileOnly
      )
        if (dataJSON.requestReceived && dataJSON.credsPresent) {
          ...
          $("#results").html(
            "Projectile and Timeular credentials were successfully sent and set. You can now " +
          ...
          $("#results")
            ...
            .addClass("alert-success");
          $("#results").html(
            "Projectile credentials and ProjectileOnly mode were successfully sent and set. You can now " +
          ...
          $("#results").text(
            "Projectile and Timeular credentials couldn't be set. Please check your input if " +
              "the credentials are correct and try again."
          );
        } else {
          console.log(
            "Something went wrong while sending and setting credentials."
          );
          $("#results")
            .removeClass("alert-success")
            .addClass("alert-warning");
          $("#results").text(
            "Projectile and Timeular credentials couldn't be sent and set. Please check your " +
