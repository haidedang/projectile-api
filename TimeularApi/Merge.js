...
class Merge {
  ...
  format(timeList) {
    for (let i = 0; i < timeList.timeEntries.length; i++) {
      const day = {};
      day['StartDate'] = timeList.timeEntries[i].duration.startedAt.substring(
        0,
        timeList.timeEntries[i].duration.startedAt.indexOf('T')
      );
      /*
       * timestamp from timeular is not accurate. sometimes 2h = 120min has a value thats != 120min. To solve that
       * I cut the seconds and milliseconds from the timestamp. Decreases precision --> alternative would
       * be to round values
       */
      day['Duration'] =
        (Date.parse(
          timeList.timeEntries[i].duration.stoppedAt.substring(
            0,
            timeList.timeEntries[i].duration.stoppedAt.lastIndexOf(':')
          )
        ) -
          Date.parse(
            timeList.timeEntries[i].duration.startedAt.substring(
              0,
              timeList.timeEntries[i].duration.startedAt.lastIndexOf(':')
            )
          )) /
        60000 /
        60;
      // get projectile packagename from timeular activity name e.g. "name": "Interne Organisation 2018 [2759-62]"
      day['Activity'] = timeList.timeEntries[i].activity.name.substring(
        timeList.timeEntries[i].activity.name.lastIndexOf('[') + 1,
        timeList.timeEntries[i].activity.name.lastIndexOf(']')
      );
  ...
  mergeDurationTime(month) {
    for (let i = 0; i < month.length; i++) {
      for (let j = i + 1; j < month.length; j++) {
        // j = i + 1 because .csv is sorted!
        // winston.debug( i + "#" + j);
        if (
          month[i]['StartDate'] === month[j]['StartDate'] &&
          month[i]['Note'].substring(0, month[i]['Note'].lastIndexOf(' #[')) ===
            month[j]['Note'].substring(0, month[j]['Note'].lastIndexOf(' #[')) &&
          ...
          const monthIId = month[i]['Note'].substring(
            month[i]['Note'].lastIndexOf(' #[') + 3,
            month[i]['Note'].lastIndexOf(']')
          );
          const monthJId = month[j]['Note'].substring(
            month[j]['Note'].lastIndexOf(' #[') + 3,
            month[j]['Note'].lastIndexOf(']')
          );
          month[i]['Note'] = month[i]['Note'].substring(0, month[i]['Note'].lastIndexOf(' #['));
          month[i]['Note'] = month[i]['Note'] + ' #[' + monthIId + ',' + monthJId + ']';
          // all fine?
          winston.debug(
            'Merge (Range) -> merging bookings --> new Note: ' + month[i]['Note'] + ' for ',
            month[i]['StartDate'],
            month[i]['Activity'],
            month[i]['Duration']
          );
  ...
  cleanArray(timeList) {
    this.sort(timeList);
    const month = this.format(timeList);
