...
const saveEntry = async(cookie, employee, time, project, note) => {
  ...
  */
  ...
  entries.forEach((item) => {
    ...
    item.includes('"What","v":"' + project + '","d"') && item.includes('"Note","v":"' + note.replace(/[\"]/g, '\\\$&') +
      '","d"')) {
      // created a new entry
      returnValue = {
        returnValue: true
...
async function setCalendarDate(date, cookie, employee) {
  ...
  // BE AWARE returned value depends on currently set date, ONLY if a date change happens the expected returned body is
  // received, else its just a "true" value within a smaller json
...
exports.getallEntriesInTimeFrame = async(startDate, endDate) => {
  ...
  await normalPostURL('POST', 'https://projectile.office.sevenval.de/projectile/gui5ajax?action=commit',
    cookie, {'values':{'Start':[{'n':'Field_TimeTrackerDate', 'v':startDate},
